...
void draw_rotated_rect(const cv::Mat &img, const cv::RotatedRect &rect, const cv::Scalar &color, int thickness)
{
    ...
    cv::line(img, vertex[1], vertex[2], cv::Scalar(255,0,0), thickness);
    cv::line(img, vertex[2], vertex[3], color, thickness);
    cv::line(img, vertex[3], vertex[0], cv::Scalar(255,0,0), thickness);*/
}
