...
std::vector<cv::RotatedRect> armor_detecter::detect_lights(bool detect_blue)
{
    ...
    cv::Mat element = cv::getStructuringElement(cv::MORPH_RECT, cv::Size(3, 3));
    cv::dilate(m_binary_color, m_binary_color, element, cv::Point(-1, -1), 1);
    m_binary_light = m_binary_color & m_binary_brightness; // tes两二值图交集
#ifdef _DEBUG_VISION
...
bool armor_detecter::detect(const cv::Mat &image, bool detect_blue)
{
    ...
    //square=test_square(image,lights);
  ...
  //  armors=armor_detecter::test_square(image,armors);
    for(auto it: armors)
        {
          draw_rotated_rect(possible_armor, it.armor, cv::Scalar(0,0,255), 2);
        //  armor_detecter::test_square(image,it.armor);
        }
...
///////////////////////将两光条间方框分割提取//////////////////////////////////////
std::vector<armor_info>  armor_detecter::test_square(const cv::Mat &img,std::vector<armor_info> &armors)
{
  std::vector<armor_info> new_armors;
  for (const auto &single_armor : armors)
  {
      auto &rect = single_armor.armor;
      cv::Point2f temp_points_ori[4];
      ...
      cv::Mat warpMatrix;
      //将方框宽度缩小0.2；
      /*temp_points[0].x=0.95*temp_points_ori[0].x+0.05*temp_points_ori[3].x;    temp_points[0].y=0.95*temp_points_ori[0].y+0.05*temp_points_ori[3].y;
      temp_points[3].x=0.95*temp_points_ori[3].x+0.05*temp_points_ori[0].x;    temp_points[3].y=0.95*temp_points_ori[3].y+0.05*temp_points_ori[0].y;
      temp_points[1].x=0.95*temp_points_ori[1].x+0.05*temp_points_ori[2].x;    temp_points[1].y=0.95*temp_points_ori[1].y+0.05*temp_points_ori[2].y;
      temp_points[2].x=0.95*temp_points_ori[2].x+0.05*temp_points_ori[1].x;    temp_points[2].y=0.95*temp_points_ori[2].y+0.05*temp_points_ori[1].y;*/
      //将方框高度扩大二倍；
      rect_points[0].x=1.7*temp_points[0].x-0.7*temp_points[1].x;   rect_points[0].y=1.7*temp_points[0].y-0.7*temp_points[1].y;
      rect_points[1].x=1.7*temp_points[1].x-0.7*temp_points[0].x;   rect_points[1].y=1.7*temp_points[1].y-0.7*temp_points[0].y;
      rect_points[2].x=1.7*temp_points[2].x-0.7*temp_points[3].x;   rect_points[2].y=1.7*temp_points[2].y-0.7*temp_points[3].y;
      rect_points[3].x=1.7*temp_points[3].x-0.7*temp_points[2].x;   rect_points[3].y=1.7*temp_points[3].y-0.7*temp_points[2].y;
      int dsth=std::sqrt((rect_points[0].x-rect_points[1].x)*(rect_points[0].x-rect_points[1].x)+(rect_points[0].y-rect_points[1].y)*(rect_points[0].y-rect_points[1].y));
      int dstw=std::sqrt((rect_points[1].x-rect_points[2].x)*(rect_points[1].x-rect_points[2].x)+(rect_points[1].y-rect_points[2].y)*(rect_points[0].y-rect_points[1].y));


        //rRect.size.height += 100;
        verdst[0] = cv::Point2f(0,dsth);
        verdst[1] = cv::Point2f(0,0);
        verdst[2] = cv::Point2f(dstw,0);
        verdst[3] = cv::Point2f(dstw,dsth);
        cv:: Mat Idst(dsth,dstw,CV_8UC1);
        cv:: Mat binar(dsth,dstw,CV_8UC1);
        warpMatrix = getPerspectiveTransform(rect_points, verdst);
        warpPerspective(img, Idst, warpMatrix, Idst.size(), CV_INTER_LINEAR, cv::BORDER_CONSTANT);
        imshow("Idst",Idst);
        cv::cvtColor(Idst,Idst,CV_BGR2GRAY);
        imshow("gray",Idst);
        cv::threshold(Idst,Idst,30,255,CV_THRESH_BINARY);
        imshow("binary",Idst);
    /////////拟合图像中椭圆轮廓；////////////////////////
        bool flag=0;
        cv::Mat pointsf;
        std::vector<std::vector<cv::Point> > contours;
        cv::Mat mask=cv::Mat::zeros(dsth,dstw,CV_8UC1);
        cv::Rect r1(1,1,dstw-2,dsth-2);
        mask(r1).setTo(255);//new_armors.push_back(single_armor);
        Idst=mask&Idst;
        imshow("mask",Idst);
        findContours(Idst, contours,CV_RETR_EXTERNAL, CV_CHAIN_APPROX_NONE);
        if(contours.size()>0)//new_armors.push_back(single_armor);
         {

          cv::Mat pointsf;
          cv::Mat cimage = cv::Mat::zeros(Idst.size(), CV_8UC3);
          int temp_x,temp_y,temp_size,temp_id;
  //遍历轮廓，存储最大面积轮廓面积，x,y;
          for(int i=0;i<contours.size();i++)
              {
                ...
                cv::RotatedRect box = fitEllipse(pointsf);
                   ellipse(cimage, box.center, box.size*0.5f, box.angle, 0, 360, cv::Scalar(0,255,255), 1, cv::LINE_AA);
                //若第0个轮廓，则把位置和面积信息记录
                   if(i==0)
                      {
                           temp_id=0;
                           temp_x=box.center.x;
                           temp_y=box.center.y;
                           temp_size=box.size.width*box.size.height;
                       }
                        //如果当前轮廓拟合椭圆外接矩形面积更大，刷新记录值
                          else {
                                 if(box.size.width*box.size.height>temp_size)
                                  {
             ...
             if( ( ((float)dstw)*0.35<(float)temp_x)
                &&((float)temp_x<((float)dstw)*0.65)
                &&( ((float)dsth)*0.35<(float)temp_y)&&((float)temp_y<((float)dsth)*0.65)
                &&(  (   ((float)temp_size)/( (float)(dstw*dsth)   )) <1   )
                &&(  ( (float)(   ((float)temp_size)/( (float)(dstw*dsth)   )))>0.3   )
                     )
             {flag=1;new_armors.push_back(single_armor);}
              printf("\n\n\n\nflag:%d\n\n\n\n",flag);
              cv::circle(cimage, cv::Point(temp_x,temp_y), 2, cv::Scalar(0, 0, 255));
              cv::imshow("contours",cimage);
            //  printf("\n\n\n\n\ns_da:%f\n\n\n\n\n",(   ((float)temp_size)/( (float)(dstw*dsth)   ))        );
