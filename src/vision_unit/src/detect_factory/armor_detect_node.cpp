...
void armor_detect_node::running(void)
{
    ...
    set_camera_exposure("/dev/video0",30);
    ...
    for (;;)
    {   rate=(cv::getTickCount()-ticks1)/cv::getTickFrequency();
        ticks1=cv::getTickCount();
        printf("%f\n",1.0/rate);
		...
		detected=false;
            ...
            std::vector<rectangdetect_info> rotatedrectangs= rectang_detector->detect_enemy(image,false);
            if(rotatedrectangs.size()!=0)
               {
                //detected=true;
                cv::solvePnP(obj_p, rotatedrectangs[0].points, camera_matrix, dist_coeffs, rvec, tvec);
                double enemy_dis = tvec.at<double>(2)/1.0;//Z distance
                double enemy_x = tvec.at<double>(0)/1.0;//x   distance
                double enemy_y = tvec.at<double>(1)/1.0;//y   distance
                std::cout<<"Enemy distance:                              "<<enemy_dis<<std::endl;
                std::cout<<"Enemy x:                                 "<<enemy_x<<std::endl;
                std::cout<<"Enemy y:                                 "<<enemy_y<<std::endl;
		armor_pos.detected = true;
		armor_pos.d = enemy_dis;
		//armor_pos.x = enemy_x;
		//armor_pos.y = enemy_y;
                    armor_pos.x = rotatedrectangs[0].rect.center.x - 320;
                    armor_pos.y = rotatedrectangs[0].rect.center.y - 240;
