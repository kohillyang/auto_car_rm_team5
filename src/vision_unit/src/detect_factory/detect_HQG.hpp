...
public:
	...
	float getlocalaveragebgr(const cv::Mat& image ,float devidex,float devidey,int bgr);
	...
	float tracking(cv::Mat frame, cv::Mat &model, cv::Rect &trackBox);

	cv::Rect fill_RotatedRect2Rect(const cv::RotatedRect &rotarect );
