...
std::vector<cv::RotatedRect> rectang_detecter::detect_lights(bool detect_blue)
{
	...
	if(detect_blue)
	{thresh_binary_color = thresh_binary_color > 37 ? 37 : thresh_binary_color;thresh_binary_color = thresh_binary_color < 35 ? 35 : thresh_binary_color;}
	else
	{thresh_binary_color = thresh_binary_color > 37 ? 37 : thresh_binary_color;thresh_binary_color = thresh_binary_color < 35 ? 35 : thresh_binary_color;}
...
bool rectang_detecter::check_light_in_Mat(const cv::Mat &image,const cv::RotatedRect &light,float para1,float para2,bool detect_blue)
{
	...
	float thread = 200;
	float bright = 0;
	int x,y,devide,count = 0,countbig = 0,total = 10;
	for(devide = total/4;devide <= total*3/4 ; devide++)
	{
		if(vertex[0].x == vertex[2].x)
		{
			x = (vertex[0].x + vertex[2].x)/2;
		}
		else
		{
			x = vertex[0].x * ((float) devide / total)
				+ vertex[2].x * (1.0 - (float) devide / total);
		}
		y = vertex[0].y * ((float) devide / total)
				+ vertex[2].y * (1.0 - (float) devide / total);
                        x = x>639?639:x;
                        y = y>479?479:y;
                        x = x<0?0:x;
                        y = y<0?0:y;
		if (detect_blue)
		{
			...
			if (image.at<cv::Vec3b>(y, x)[0] > thread)
				countbig++;
		}
		else
		{
			bright += image.at<cv::Vec3b>(y, x)[2];
			if (image.at<cv::Vec3b>(y, x)[2] > thread)
				countbig++;
		}
		count++;

		if(vertex[1].x == vertex[3].x)
		{
			x = (vertex[1].x + vertex[3].x)/2;
		}
		else
		{
			x = vertex[1].x * ((float) devide / total)
				+ vertex[3].x * (1.0 - (float) devide / total);
		}
		y = vertex[1].y * ((float) devide / total)
				+ vertex[3].y * (1.0 - (float) devide / total);
                        x = x>639?639:x;
                        y = y>479?479:y;
                        x = x<0?0:x;
                        y = y<0?0:y;
		...
		float thresh_min_area, float thresh_max_area)
{
	...
	for (const auto &rect : lights)
	{
		...
		if (h >= 3 &&!(rect.center.y<120&&rect.size.height>80))
		{
			//std::cout<<"check_light_in_Mat  x:"<<rect.center.x<<"  y:"<<rect.center.y<<"  area:"<<rect.size.area()
...
//							&& rect.size.area() <= thresh_max_area)
					if (h*w >= thresh_min_area
												&& h*w <= thresh_max_area)
					{
						if(check_light_in_Mat(m_image,rect,160,0.2,detect_blue))
						{
...
//				if (hwratio < 1.5)
//				{
//					if (check_light_in_Mat(m_image, rect, 170, 0.3,detect_blue))
//					{
		...
		float &h)
{
	...
	w = point_distance(vertex[0], vertex[1]);w = w==0?1:w;
	h = point_distance(vertex[1], vertex[2]);h = h==0?1:h;
	float angle = 0;
	if (w > h)
	{
		std::swap(w, h);
...
float rectang_detecter::getlocalaveragebgr(const cv::Mat& image ,float devidex,float devidey,int bgr)
{
	int nr = image.rows; // number of rows  hang
	int nc = image.cols ;//* image.channels(); // total number of elements per line  lie
	float totalgray = 0;
	int count=0;
	for (int y = (int)(nr*0.5 - nr*devidey*0.5); y < (int)(nr*0.5 + nr*devidey*0.5); y++)
	{
		for (int x = (int)(nc*0.5 - nc*devidex*0.5); x < (int)(nc*0.5 + nc*devidex*0.5); x++)
		{
			totalgray += image.at<cv::Vec3b>(y, x)[bgr];
			count++;
		}
	}
	totalgray = totalgray/(count+1);
		...
		const std::vector<cv::RotatedRect> &lights)
{
	...
	for (const auto &light1 : lights)
	{
		for (const auto &light2 : lights)
		{
			...
			/*以下为检测装甲板*/
			if ((fabs(ang1 - ang2) < 25 && fabs(h1 - h2) < 0.3 * h_
			//&&fabs(w1-w2)<w_/4
					)
				||(h_< 12
				&&fabs(ang1 - ang2) < 40 && fabs(h1 - h2) < 0.5 * h_
					))//delta h , w ,ang
			{
						...
						))
				{
					...
					for (int i=0;i<4;i++)
					{
						if((vertex[i].x<=rect.center.x)&&(vertex[i].y<=rect.center.y))
						  {
							vecpoint[0]=vertex[i];
						  }
						if((vertex[i].x>=rect.center.x)&&(vertex[i].y<=rect.center.y))
					...
					if(0 <= lx && 0 <= ly && lw > 0 && lh > 0 && lx + lw <= m_image.cols && ly + lh <= m_image.rows)
					{
						...
						lost3 = - getlocalaveragebgr(lrect, 1, 1, 1);
					}
					float totallost = lost1 + lost2 + lost3/10 - 0.5 * h_;
                         
                                        if(rect.center.y<160&&rect.size.width>90)
                                        {}
                                        else
                                        {
...
float rectang_detecter::tracking(cv::Mat frame, cv::Mat &model, cv::Rect &trackBox)
{
    cv::Mat search_area;
    //cvtColor(frame, search_area, CV_RGB2GRAY);

    cv::Rect searchWindow;
    searchWindow.width = trackBox.width * 2;
    searchWindow.height = trackBox.height * 2;
    ...
    frame(searchWindow).copyTo(search_area);
    ...
    matchTemplate(search_area, model, similarity, CV_TM_CCOEFF_NORMED);

    double mag_r;
    cv::Point point;
    cv::minMaxLoc(similarity, 0, &mag_r, 0, &point);
    std::cout<<"                                    "<<"delta x:"<<point.x + searchWindow.x - trackBox.x
    		<<" \n "<<"                                    "<<mag_r<<std::endl;
    trackBox.x = point.x + searchWindow.x;
    trackBox.y = point.y + searchWindow.y;
    //frame(trackBox).copyTo(model);
    return mag_r;
}
cv::Rect rectang_detecter::fill_RotatedRect2Rect(const cv::RotatedRect &rotarect )
{
	cv::Rect rect;
	rect.width = 2.2 * rotarect.size.width;
	rect.height = 2.2 * rotarect.size.height;
	rect.x = rotarect.center.x - rect.width/2;
	rect.y = rotarect.center.y - rect.height/2;
	return rect;
}
std::vector<rectangdetect_info> rectang_detecter::detect_enemy( const cv::Mat &image, bool detect_blue)
{
	...
	m_image = image;
	static cv::Rect box(0,0,0,0);
	static cv::Mat modle;
	static int untrackcount = 0;
	static int trackcount = 0;
	static rectangdetect_info lastrectang;
#ifdef _DEBUG_HQG
...
#ifdef _DEBUG_HQG
	draw_rotated_rects(light_img, lights, cv::Scalar(0, 255, 0), 1, false,cv::Scalar(255, 0, 0));
#endif
	...
	if(Rectangs.size()!=0)
	{
		lastrectang = *finalrect;
		box = fill_RotatedRect2Rect(finalrect->rect);
		box &= cv::Rect(0, 0, m_image.cols, m_image.rows);
		m_image(box).copyTo(modle);
		trackcount = 0;
		untrackcount = 0;
	}
	if(Rectangs.size()==0)
	{
		if (!(box.x == 0 && box.y == 0) && !modle.empty())
		{
			//imshow("modle0", modle);
			if (tracking(m_image, modle, box) > 0.53 )
			{
				if(trackcount>=25)
				{
					box = cv::Rect(0, 0, 0, 0);
					...
					trackcount++;
				finalrect = &lastrectang;
				finalrect->rect.center.x = box.x+box.width/2;
				finalrect->rect.center.y = box.y+box.height/2;
				Rectangs.push_back(lastrectang);
				cv::rectangle(m_image, box, cv::Scalar(255, 255, 255), 1);
				untrackcount = 0;
			}
			else
			{
...
//		cv::putText(final_rectang, str, finalrect->rect.center, CV_FONT_ITALIC, 1.5, cv::Scalar(55, 250, 0), 5, 8);
	...
	//draw_rotated_rect(m_image, rectangdetect_info::car_rect, cv::Scalar(250,100,0), 1);
	...
	cv::line(m_image, Rectangs[0].rect.center, cv::Point2f(320,240), cv::Scalar(255, 100, 100), 2);
	}
